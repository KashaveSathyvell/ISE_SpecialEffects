...
class Enemy(pygame.sprite.Sprite):
    ...
    def take_damage(self, amount):
        if self.state not in ["hurt", "death"]:
            ...
            if self.health <= 0:
                self.state = "death"
                try:
                    if not hasattr(self, 'death_sound'):
                        self.death_sound = pygame.mixer.Sound(os.path.join("src", "assets", "sounds", "enemy_death.mp3"))
                        self.death_sound.set_volume(0.4)  # Adjust volume as needed
    ...
    def can_attack(self):
        ...
        try:
            if not hasattr(self, 'enemyAtt_sound'):
                self.enemyAtt_sound = pygame.mixer.Sound(os.path.join("src", "assets", "sounds", "enemyAttack.mp3"))
                self.enemyAtt_sound.set_volume(0.8)  # Adjust volume as needed
            self.enemyAtt_sound.play()
        except pygame.error as e:
            print(f"Error playing enemyAtt_sound sound: {e}")
...
class Golem(Enemy):
        ...
        if self.state not in ["hurt", "death"]:
            ...
            if self.health <= 0:
                ...
                try:
                    if not hasattr(self, 'death_sound'):
                        self.death_sound = pygame.mixer.Sound(os.path.join("src", "assets", "sounds", "enemy_death.mp3"))
                        self.death_sound.set_volume(0.6)  # Adjust volume as needed
                    self.death_sound.play()
                except pygame.error as e:
                    print(f"Error playing enemy death sound: {e}")
            else:
    ...
    def can_attack(self):
        ...
        try:
            if not hasattr(self, 'GolemAttack'):
                self.GolemAttack = pygame.mixer.Sound(os.path.join("src", "assets", "sounds", "enemyGolemAttack.mp3"))
                self.GolemAttack.set_volume(0.4)  # Adjust volume as needed
            self.GolemAttack.play()
        except pygame.error as e:
            print(f"Error playing GolemAttack sound: {e}")
...
class Boss2(Enemy):
    ...
    def fire_projectile(self, attack_state, player):
        ...
        try:
            if not hasattr(self, 'rangeAtt_sound'):
                self.rangeAtt_sound = pygame.mixer.Sound(os.path.join("src", "assets", "sounds", "Boss2_MagicAtt.wav"))
                self.rangeAtt_sound.set_volume(0.5)  # Adjust volume as needed
            self.rangeAtt_sound.play()
        except pygame.error as e:
            print(f"Error playing projectile sound: {e}")
    ...
    def become_angry(self):
        ...
        try:
            if not hasattr(self, 'angry_sound'):
                self.angry_sound = pygame.mixer.Sound(os.path.join("src", "assets", "sounds", "Boss_Angry.mp3"))
                self.angry_sound.set_volume(0.7)  # Adjust volume as needed
            self.angry_sound.play()
        except pygame.error as e:
            print(f"Error playing boss angry sound: {e}")
        ...
        if self.state not in ["death"]:  # Can now take damage if not dead
            if not self.is_invincible:  # Only take damage if not invincible
                ...
                if self.health <= 0:
                    self.state = "death"
                    try:
                        if not hasattr(self, 'boss2_death'):
                            self.boss2_death = pygame.mixer.Sound(os.path.join("src", "assets", "sounds", "Boss2_death.wav"))
                            self.boss2_death.set_volume(0.4)  # Adjust volume as needed
                        self.boss2_death.play()
                    except pygame.error as e:
                        print(f"Error playing boss2_death sound: {e}")
                else:
                    self.previous_state = self.state  # Store previous state
                    self.state = "hurt"
                    try:
                        if not hasattr(self, 'boss2_hurt'):
                            self.boss2_hurt = pygame.mixer.Sound(os.path.join("src", "assets", "sounds", "Boss2_hurt.wav"))
                            self.boss2_hurt.set_volume(0.4)  # Adjust volume as needed
                        self.boss2_hurt.play()
                    except pygame.error as e:
                        print(f"Error playing boss2_hurt sound: {e}")
    ...
    def can_attack(self):
        ...
        if regular_attack and self.current_frame == 3:
            try:
                if not hasattr(self, 'Bossattack_sound'):
                    self.Bossattack_sound = pygame.mixer.Sound(os.path.join("src", "assets", "sounds", "Boss2_attack.ogg"))
                    self.Bossattack_sound.set_volume(0.6)  # Adjust volume as needed
                self.Bossattack_sound.play()
            except pygame.error as e:
                print(f"Error playing boss attack sound: {e}")
...
class Boss1(Enemy):
    ...
    def update(self, dt, player=None):
        ...
        if self.check_player_after_hurt and self.current_player:
            ...
            if dist_to_player <= self.detection_radius:
                ...
                try:
                    if not hasattr(self, 'Boss1Target_sound'):
                        self.Boss1Target_sound = pygame.mixer.Sound(os.path.join("src", "assets", "sounds", "Boss1_GolemSound.wav"))
                        self.Boss1Target_sound.set_volume(0.6)  # Adjust volume as needed
                    self.Boss1Target_sound.play()
                except pygame.error as e:
                    print(f"Error playing boss attack sound: {e}")
    ...
    def perform_attack(self, player):
        ...
        try:
            if not hasattr(self, 'Boss1Att_sound'):
                self.Boss1Att_sound = pygame.mixer.Sound(os.path.join("src", "assets", "sounds", "Boss1_Attack.mp3"))
                self.Boss1Att_sound.set_volume(0.6)  # Adjust volume as needed
            self.Boss1Att_sound.play()
        except pygame.error as e:
            print(f"Error playing boss attack sound: {e}")
    ...
    def take_damage(self, amount):
        ...
        if self.state not in ["death", "hurt"]:  # Only take damage if not already hurt or dead
            ...
            if self.health <= 0:
                ...
                try:
                    if not hasattr(self, 'Boss1Death_sound'):
                        self.Boss1Death_sound = pygame.mixer.Sound(os.path.join("src", "assets", "sounds", "Boss1_dead.mp3"))
                        self.Boss1Death_sound.set_volume(0.6)  # Adjust volume as needed
                    self.Boss1Death_sound.play()
                except pygame.error as e:
                    print(f"Error playing boss death sound: {e}")
                ...
                try:
                    if not hasattr(self, 'Boss1hurt_sound'):
                        self.Boss1hurt_sound = pygame.mixer.Sound(os.path.join("src", "assets", "sounds", "Boss1_hurt.mp3"))
                        self.Boss1hurt_sound.set_volume(0.6)  # Adjust volume as needed
                    self.Boss1hurt_sound.play()
                except pygame.error as e:
                    print(f"Error playing boss death sound: {e}")
